...
async def test_create_and_get_by_id(db_session):  # noqa: F811
    ...
    doc = await repo.create(
        DocumentCreate(filename="doc.pdf", file_type=".pdf", file_size=500, file_hash="hash123")
    )
...
async def test_get_by_hash(db_session):  # noqa: F811
    repo = DocumentRepository(db_session)
    await repo.create(
        DocumentCreate(filename="a.pdf", file_type=".pdf", file_size=100, file_hash="uniquehash")
    )
...
async def test_list_with_pagination(db_session):  # noqa: F811
    ...
    for i in range(5):
        await repo.create(
            DocumentCreate(filename=f"file{i}.txt", file_type=".txt", file_size=10, file_hash=f"h{i}")
        )
