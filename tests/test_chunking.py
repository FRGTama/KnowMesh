...
_DOC_ID = UUID("aaaaaaaa-aaaa-4aaa-aaaa-aaaaaaaaaaaa")
_DOC_ID2 = UUID("bbbbbbbb-bbbb-4bbb-bbbb-bbbbbbbbbbbb")


def test_recursive_chunker_short():
    doc = Document(text="Hello world", metadata={"filename": "test.txt", "document_id": str(_DOC_ID)})
...
def test_recursive_chunker_splits():
    text = "word " * 600
    doc = Document(text=text, metadata={"filename": "test.txt", "document_id": str(_DOC_ID)})
    chunks = RecursiveChunker(window_size=512, overlap=0).chunk(doc)
    assert len(chunks) >= 2
    assert all(c.strategy == "recursive" for c in chunks)
    assert {c.document_id for c in chunks} == {_DOC_ID}
    indices = [c.index for c in chunks]
    assert indices == list(range(len(chunks)))


def test_recursive_chunker_overlap():
    doc = Document(text="word " * 600, metadata={"filename": "test.txt", "document_id": str(_DOC_ID)})
...
def test_empty_document():
    doc = Document(text="", metadata={"filename": "empty.txt", "document_id": str(_DOC_ID)})
    assert RecursiveChunker().chunk(doc) == []
    assert SemanticChunker().chunk(doc) == []


def test_chunk_metadata_contains_document_id():
    doc = Document(text="Hello", metadata={"filename": "test.txt", "document_id": str(_DOC_ID2)})
    chunks = RecursiveChunker().chunk(doc)
    assert chunks[0].metadata["document_id"] == str(_DOC_ID2)
    assert chunks[0].document_id == _DOC_ID2
